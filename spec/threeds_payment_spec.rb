...
RSpec.describe 'Iyzipay' do
  ...
  it 'should create payment with physical and virtual item for standard merchant' do
    ...
    begin
      ...
      unless threeds_initialize['threeDSHtmlContent'].nil?
        $stdout.puts Base64.decode64(threeds_initialize['threeDSHtmlContent']).inspect
        expect(threeds_initialize['status']).to eq('success')
        expect(threeds_initialize['locale']).to eq('tr')
        expect(threeds_initialize['systemTime']).not_to be_nil
      end
